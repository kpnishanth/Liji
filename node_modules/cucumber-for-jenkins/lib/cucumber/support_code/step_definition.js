function StepDefinition(pattern, code) {
  ...
  var self = {
    ...
    invoke: function invoke(step, world, scenario, stepDomain, callback) {
      ...
      codeCallback.skip = function skip(reason) {
        Cucumber.Debug.notice('stepdef calling back (via callback.skip())\n', 'Cucumber.SupportCode.StepDefinition', 5);
        var skippedStepResult = Cucumber.Runtime.SkippedStepResult({step: step, pendingReason: reason, attachments: scenario.getAttachments()});
        cleanUp();
        callback(skippedStepResult);
      };
